...
| PATCH | `/api/donations/:id` | `{ status: "paid" }`: confirma o pagamento de uma doação pendente (**administração**). O valor entra em `raised` da campanha e conta um apoiador. Responde `{ donation, campaign }` | 200, 401, 403, 404, 409, 422 |
