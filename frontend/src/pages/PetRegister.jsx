...
import { Alert, Button, Checkbox, Chip, TextField } from '../components/ui'
import { createPet } from '../services/petsService'

const KEY_STORAGE = 'adopet-admin-key'
const MAX_TAGS = 5
...
function toPayload(values) {
  ...
  return {
    ...
    tags: TAGS.filter((tag) => values.tags.includes(tag.key)).map((tag) => tagLabel(tag, values.sex)),
...
export default function PetRegister() {
  ...
  function toggleTag(key) {
    setValues((current) => {
      ...
      if (!selected && current.tags.length >= MAX_TAGS) return current
  ...
  async function handleSubmit(event) {
    ...
    saveKey(rememberKey ? adminKey.trim() : '')
    setSending(true)
    try {
      const pet = await createPet(toPayload(values), adminKey.trim())
      setValues(EMPTY)
      setErrors({})
      showFeedback({ tone: 'success', title: `${pet.name} foi cadastrado`, pet })
    } catch (error) {
      if (error.status === 401) {
        ...
        showFeedback({ tone: 'danger', title: 'Não foi possível cadastrar', text: error.message })
      }
    } finally {
      setSending(false)
    }
  ...
  return (
    <section className="section section--tight">
      <form className="form-card" onSubmit={handleSubmit} noValidate>
        <div className="stack">
          <h1 className="t-heading-lg">Cadastrar animal</h1>
        </div>
        ...
        {feedback && (
          <Alert tone={feedback.tone} title={feedback.title}>
            {feedback.pet ? (
              <>
                Ele já está na lista de adoção. <Link to={`/pets/${feedback.pet.id}`}>Ver perfil de {feedback.pet.name}</Link>
              </>
            ) : (
              feedback.text
            )}
        ...
        <fieldset className="form-section">
          ...
          <div className="stack" style={{ gap: 8 }}>
            ...
            <p className="ap-field__hint" style={errors.tags ? { color: 'var(--danger)' } : undefined}>
              {errors.tags ?? `Até ${MAX_TAGS}. Aparecem no card do animal.`}
            </p>
          </div>
          <TextField
            label="História"
        ...
        <div className="form-actions">
          <Button variant="ghost" onClick={() => { setValues(EMPTY); setErrors({}); setFeedback(null) }}>
            Limpar
          </Button>
          <Button type="submit" disabled={sending}>{sending ? 'Cadastrando…' : 'Cadastrar animal'}</Button>
        </div>
      </form>
    </section>
  )
